...
/**
 * Afkir Qibla – HOTFIX build-safe App.jsx
 * - Guards around all sensors & API
 * - Countdown shows hours + minutes
 * - Safe-area padding + theme button reachable
 * - Kaaba fixed, needle points to Kaaba
 * - Backgrounds optional (won't crash if missing)
 */
...
export default function App(){
  const { coords, error, loading, permission, request } = useGeolocation()
  ...
  useEffect(() => {
    const id = setInterval(()=> setBgIdx(i => (i+1)%BACKGROUNDS.length), 30000)
    return () => clearInterval(id)
  }, [])

  // midnight refresh + countdown tick
  useEffect(() => {
  ...
  return (
    <div style={{minHeight:"100dvh", color:"var(--fg)", backgroundSize:"cover", backgroundPosition:"center", backgroundImage:`linear-gradient(rgba(4,6,12,.65), rgba(4,6,12,.65)), url(${bg})`, transition:"background-image .8s ease"}}>
      <style>{`
        ...
        .container { padding: calc(env(safe-area-inset-top) + 12px) 16px 16px; font-family: system-ui, -apple-system, Segoe UI, Roboto, sans-serif; }
