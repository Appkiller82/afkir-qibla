...
/**
 * Afkir Qibla — FULL FIX (inkl. siste oppdatering)
 * - Maliki/EGAS (method=5, school=0) via Aladhan
 * - Robust parsing: setter BÅDE timer og minutter eksplisitt (fikser 09:xx og "2017 t"-bug)
 * - "Neste bønn: NAME kl HH:MM (om X t Y min)" + sanntid (30s)
 * - Auto-refresh ved midnatt og når du flytter deg > 5 km (GPS watch)
 * - By + koordinater (reverse geocoding)
 * - Kompass: "Tillat kompass" + hjelpmodal; Kaaba fast; viser peker riktig
 * - Kart-undermeny (Leaflet via CDN) som fallback
 * - Adhan test + enkle fane-varsler (knapp blir grønn når aktiv)
 * - Tema-knapp mellom tittel og dato, husker valg
 * - Bakgrunnsrotasjon (valgfritt)
 */
...
// ---------- Aladhan (Maliki) — robust (sets H+M) ----------
              ...
              <>
                ...
                <div className="row" style={{marginTop:10}}>
                  <button className={remindersOn ? "btn btn-green" : "btn"} onClick={async ()=>{
                    await (async ()=>{ if ("Notification" in window && Notification.permission==="default") { try { await Notification.requestPermission() } catch {} } })();
